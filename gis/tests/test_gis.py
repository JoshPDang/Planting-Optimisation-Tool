...
def test_polygon():
    ...
    return [
        [(-8.55, 125.57), (-8.56, 125.57), (-8.56, 125.58), (-8.55, 125.58), (-8.55, 125.57)]
    ]
...
def test_parse_geometry_auto_detect(gee_initialized, test_point):
    ...
    # Polygon
    polygon = [
        [(-8.55, 125.57), (-8.56, 125.57), (-8.56, 125.58), (-8.55, 125.57)]
    ]
...
def test_get_texture(gee_initialized, test_point):
    ...
    if texture is not None:
        ...
        print(f"  Note: Currently extracting pH value to demonstrate GEE capability")
        print(f"  Replace soil_texture config with actual texture asset for production use")
    else:
...
def test_build_farm_profile_point(gee_initialized, test_point):
    ...
    required_fields = [
        "id", "year", "rainfall_mm", "temperature_celsius", "elevation_m",
        "slope_degrees", "soil_ph", "area_ha", "latitude", "longitude", "coastal"
    ]
...
def test_coastal_flag_logic(gee_initialized):
    ...
    if profile["elevation_m"] is not None and profile["rainfall_mm"] is not None:
        expected_coastal = (
            profile["elevation_m"] < 100 and 
            500 <= profile["rainfall_mm"] <= 3000
        )
...
def test_dataset_validation_quality(gee_initialized, test_point):
    ...
    print(f"        Polygon aggregation provides no improvement over centroid")
...
def test_soil_ph_warning(gee_initialized, test_point):
    ...
    if ph is not None:
        ...
        print(f"  Data quality (EDA): r=0.18, MAE=1.21 pH units - POOR")
        print(f"  Problem: Severe value compression (5.5-6.5 vs actual 5-8 range)")
        print(f"  Status: NOT RECOMMENDED for operational use in Timor-Leste")
        print(f"\n  EDA Recommendation:")
        print(f"  - Develop local calibration model using PO field measurements")
        print(f"  - Use predictors: elevation (r=0.98), rainfall, geology, land cover")
        print(f"  - OR exclude pH from analysis until better data available")
    else:
...
def test_eda_validation_summary(gee_initialized, test_point):
    ...
    for var, dataset, corr, mae, status in datasets:
        print("{:<20} {:<15} {:<15} {:<12} {:<15}".format(
            var, dataset, corr, mae, status
        ))
...
def test_all_datasets_configured():
    ...
    expected = ["rainfall", "elevation", "temperature", "soil_ph", "soil_texture", "dem"]
...
def test_bulk_create_profiles(gee_initialized):
    ...
    required_fields = ["id", "year", "rainfall_mm", "temperature_celsius", 
                      "elevation_m", "status"]
    for field in required_fields:
...
def test_update_all_fields(gee_initialized, test_point):
    ...
    required_fields = ["id", "year", "rainfall_mm", "temperature_celsius",
                      "elevation_m", "slope_degrees", "soil_ph", "area_ha"]
